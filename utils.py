...
def train_test_split_scale(data, train_size=0.8, datetime_col='datetime', target_col='value'):
    ...
    scaler = MinMaxScaler()
    train_data['energy_scaled'] = scaler.fit_transform(train_data[[target_col]])
    test_data['energy_scaled'] = scaler.transform(test_data[[target_col]])
    train_data.drop(columns=[target_col], inplace=True)
    test_data.drop(columns=[target_col], inplace=True)
...
def create_sliding_window(client_data, energy_col = 'energy_scaled', window_size = 24, step_size = 1):
    client_windows = []
    client_targets = []
    ...
    for i in range(0, num_readings - window_size, step_size):
        ...
        window = client_data.iloc[window_start:window_end + 1][energy_col].values
        target = client_data.iloc[prediction_index][energy_col]
